...
function runAllTests() {
  ...
  // Test 1: Verifikasi Struktur 6 Sheet
  try {
    var expectedSheets = [
    ...
    if (missing.length === 0) {
      results.push({ name: '1. Struktur 6 Sheet Opsi B', status: 'PASS', message: 'Semua 6 sheet terdaftar lengkap.' });
    } else {
      results.push({ name: '1. Struktur 6 Sheet Opsi B', status: 'WARN', message: 'Sheet belum dibuat di spreadsheet: ' + missing.join(', ') });
    }
  } catch (e) {
  ...
  try {
    ...
    if (dash && dash.success && dash.data) {
      results.push({ name: '2. Handler Dashboard & Kalkulasi 20 JP', status: 'PASS', message: 'Dashboard berhasil dimuat. Total data: ' + dash.data.total_kompetensi });
    } else {
